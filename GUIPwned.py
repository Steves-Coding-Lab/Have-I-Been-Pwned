...
main_win.configure(bg='#383B40')
frame = Frame(main_win)
frame.configure(bg='#383B40')
...
def check_pass():
    ...
    if response == "This password isn't in the list":
        pwned_output = Label(frame,text="Good news! - No pwnaged found.", fg ="#69FF69", bg="#383B40", font=('Arial',16))
        ...
        pwned_output = Label(frame,text="Your Password has been Pwned!", fg ="#FF5A5A", bg="#383B40", font=('Arial',16))
        output = Label(frame,text="This password has previously appeared in a data breach and should never be used. There is "+response+" instances of this password in the Have I been pwned database.",
                       fg ="#FF5A5A", bg="#383B40", wraplength=400, font=('Arial',14))
        pwned_output.grid(row=4, column=0, columnspan=2, pady=10)
        output.grid(row=5, column=0, columnspan=2, pady=0)    
...
#Creating Widgets
title_label = Label(frame,text="Have I Been Pwned?", fg ="#F08080", bg="#383B40", font=('Arial',30))
...
var1 = IntVar()
show_pass = Checkbutton(frame, text='Show Password?', font=('Arial', 12), selectcolor="#F08080", fg ="white", bg="#383B40",  activeforeground="white",
                        variable=var1, onvalue=1, offvalue=0, command=show_password)
check_button = Button(frame, text="Check Password", fg="white", bg="#F08080", activebackground="#B34545", activeforeground="white", font=('Arial', 14), command=check_pass)
